      ...
      <div className="space-y-12">
        ...
        {isEditing && (
          <div className="space-y-8 animate-fade-in">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-gray-50 dark:bg-gray-900/50 p-6 rounded-[2.5rem] border border-gray-100 dark:border-gray-800">
              ...
              {Object.entries(groupedCategories).map(([type, groups]) => (
                <div key={type} className="space-y-6">
                  <div className="flex items-center gap-4">
                    <h2 className="text-sm font-black uppercase tracking-[0.3em] text-primary-500">{type} Categories</h2>
                    <div className="h-px flex-1 bg-primary-100 dark:bg-primary-900/30"></div>
                  </div>

                  {Object.entries(groups).map(([group, cats]) => (
                    <div key={group} className="card p-8 rounded-[2.5rem]">
                      <h3 className="text-xl font-black mb-6 text-gray-900 dark:text-white tracking-tight capitalize">
                        {group}
                      </h3>
                      ...
                      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {cats.map((cat) => {
                          ...
                          return (
                            <div
                              key={cat._id}
                              className="group flex flex-col gap-4 bg-gray-50/50 dark:bg-gray-800/20 p-5 rounded-[2rem] border border-transparent hover:border-primary-500/20 hover:bg-white dark:hover:bg-gray-800/40 transition-all duration-300"
                            >
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                  <div className={`p-3 rounded-2xl ${type === "Income"
                                    ...
                                    } shadow-lg shadow-current/10 transition-transform group-hover:scale-110`}>
                                ...
                                <button
                                  ...
                                  className="p-2 rounded-xl text-gray-300 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/30 transition-all"
                                >
                                  <Trash2 size={18} />
                                </button>
                              </div>
                              <div className="relative">
                                ...
                                <input
                                  ...
                                  placeholder="0"
                                  className="input-field pl-10 h-14 text-lg font-black tracking-tighter"
                                />
                              </div>
                            </div>
                          );
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              {[
                ...
                <div key={i} className={`card p-8 rounded-[2.5rem] border-b-8 border-${stat.color}-500 transition-transform hover:-translate-y-1`}>
                  <p className="text-[11px] font-black uppercase tracking-[0.2em] text-gray-400 mb-2">{stat.label}</p>
                  <h3 className={`text-4xl font-black tracking-tighter text-gray-900 dark:text-white`}>
            ...
            {/* Action Buttons */}
            <div className="flex justify-center md:justify-end gap-4 p-4">
              <button
                onClick={() => {
                  setIsEditing(false);
                  setEditableData(planData || {});
                }}
                className="btn-secondary px-10 group"
              >
              ...
              <button
                ...
                disabled={isPending}
                className="btn-primary px-10 shadow-2xl shadow-primary-500/30 group"
              >
                <Save size={20} className="group-hover:scale-110 transition-transform" />
                <span>{isPending ? "Saving Strategy..." : "Commit Plan"}</span>
              </button>
            </div>
          </div>
        )}
