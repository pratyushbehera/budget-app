...
const GroupSummary = ({ group, groupId }) => {
  ...
  if (isLoading) return (
    <div className="p-8 bg-gray-50 dark:bg-gray-900/40 rounded-[2.5rem] border-2 border-dashed border-gray-200 dark:border-gray-800 animate-pulse">
      <p className="text-center text-gray-400 font-black uppercase tracking-widest text-xs">Summarizing finances...</p>
    </div>
  );
        ...
        <div className="relative z-10 space-y-6">
          <div className="flex justify-between items-start">
            ...
            <div className="text-right opacity-80">
              <p className="text-[10px] font-black uppercase tracking-widest">Group total</p>
              <p className="text-2xl font-black tracking-tight">₹{summary?.totalPaid?.reduce((acc, m) => acc + m.amount, 0).toFixed(2)}</p>
            </div>
          </div>

          <div className="space-y-1">
            <h2 className="text-4xl font-black tracking-tighter">Fully Settled</h2>
  ...
  return (
    <div className="space-y-6">
      <div className={`rounded-[2.5rem] p-8 text-white shadow-2xl transition-all duration-500 hover:scale-[1.02]
        ${yourBalance === 0 ? "bg-gradient-to-br from-indigo-500 to-blue-600 shadow-indigo-500/20" :
          yourBalance < 0 ? "bg-gradient-to-br from-rose-500 to-pink-600 shadow-rose-500/20" :
            "bg-gradient-to-br from-emerald-500 to-teal-600 shadow-emerald-500/20"}`}>
        <div className="flex justify-between items-start">
          <div className="space-y-1">
            <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-80 mb-2">Group Balance Insight</p>
          ...
          <div className="text-right">
            <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-80 mb-2">Group Total</p>
            <p className="text-2xl font-black tracking-tighter">
              ₹{summary?.totalPaid?.reduce((sum, m) => sum + m.amount, 0).toFixed(2)}
            </p>
          </div>
        </div>
        ...
        <div className="mt-8 pt-7 border-t border-white/15">
          <div className="flex items-center gap-2 mb-4 opacity-80">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.2em]">Spending Breakdown</p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {summary?.totalPaid?.sort((a, b) => b.amount - a.amount).map((m) => (
              <div key={m.id} className="group/item flex justify-between items-center bg-white/10 hover:bg-white/15 p-3.5 rounded-2xl transition-all duration-300 border border-white/5 hover:border-white/10">
                <div className="flex items-center gap-3 overflow-hidden">
                  ...
                  <div className="flex flex-col">
                    <span className="text-xs font-black truncate">{m.name}{m.id === user._id ? " (You)" : ""}</span>
                    <span className="text-[9px] font-bold opacity-60 uppercase tracking-tighter">Contribution</span>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-black tracking-tight shrink-0">₹{m.amount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                </div>
              </div>
            ))}
          </div>
            ...
            >
              ...
              <div className="flex items-center gap-4">
                <div className="relative">
                  ...
                  <div className={`absolute -right-1 -bottom-1 w-4 h-4 rounded-full border-2 border-white dark:border-gray-800 ${isYouOwe ? "bg-rose-500" : "bg-emerald-500"}`}></div>
                </div>
              ...
              <div className="text-right flex items-center gap-4">
                <p className={`text-2xl font-black tracking-tighter ${isYouOwe ? 'text-rose-500' : 'text-emerald-500'}`}>
                  ₹{s.amount.toFixed(2)}
                </p>
