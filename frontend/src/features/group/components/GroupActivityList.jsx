...
const GroupActivityList = ({ groupId }) => {
  ...
  if (isLoading)
    return (
      <div className="p-8 bg-gray-50 dark:bg-gray-900/40 rounded-[2.5rem] border-2 border-dashed border-gray-200 dark:border-gray-800 animate-pulse">
        <p className="text-center text-gray-400 font-black uppercase tracking-widest text-xs">Loading activity feed...</p>
      </div>
    );
  ...
  const getTypeBadge = (type) => {
    switch (type) {
      case "invite":
        return { label: "Invite", cls: "bg-blue-50 text-blue-600 border-blue-100" };
      case "transaction":
      case "transaction_deleted":
      case "transaction_edited":
        return { label: "Money", cls: "bg-emerald-50 text-emerald-600 border-emerald-100" };
      case "removed":
        return { label: "System", cls: "bg-rose-50 text-rose-600 border-rose-100" };
      case "left":
        return { label: "Exit", cls: "bg-orange-50 text-orange-600 border-orange-100" };
      case "settle":
        return { label: "Settlement", cls: "bg-purple-50 text-purple-600 border-purple-100" };
      default:
        return { label: "Update", cls: "bg-gray-100 text-gray-600 border-gray-200" };
    }
  };
    ...
    <div className="bg-white dark:bg-gray-900/40 rounded-[2.5rem] shadow-xl shadow-gray-200/50 dark:shadow-none p-8 border border-gray-100 dark:border-gray-800 animate-fade-in">
      ...
      {!activities.length ? (
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="text-lg font-black text-gray-400 tracking-tight">Quiet so far</p>
          <p className="text-xs text-gray-400 uppercase tracking-widest mt-1">Updates will appear here</p>
        </div>
      ) : (
        <div className="space-y-6 max-h-[40vh] overflow-y-auto pr-2 custom-scrollbar">
          {activities.map((act) => {
            ...
            let text = "";
            if (act.type === "invite") text = `${actor} invited ${act.data.email}`;
            if (act.type === "transaction") text = `${actor} added a transaction of ₹${act.data.amount}`;
            if (act.type === "removed") text = `${actor} removed a member`;
            if (act.type === "left") text = `${actor} left the group`;
            if (act.type === "settle") text = `${actor} settled ₹${act.data.amount}`;
            if (act.type === "transaction_deleted") text = `${actor} deleted a transaction of ₹${act.data.amount}`;
            if (act.type === "transaction_edited") text = `${actor} edited a transaction`;

            return (
              <div
                key={act._id}
                className="group flex flex-col gap-3 p-5 rounded-[2rem] bg-gray-50/50 dark:bg-gray-800/20 border border-transparent hover:border-gray-100 dark:hover:border-gray-700 transition-all duration-300"
              >
